...
LinkedList.prototype.search = function (arg) {
  ...
  if (typeof arg === "function") {
    while (this.current.next) {
      if (arg(this.current.value)) {
        return this.current.value;
      } else {
        this.current = this.current.next;
      }
    }
    if (arg(this.current.value)) return this.current.value;
  } else {
    while (this.current.next) {
      if (this.current.value == arg) {
        return arg;
      } else {
        this.current = this.current.next;
      }
    }
    if (this.current.value == arg) return arg;
  }
  return null;
};
...
HashTable.prototype.hasKey = function (key) {
  if (this.buckets.hasOwnProperty(this.hash(key))) {
    ...
    return bucket.hasOwnProperty(key) ? true : false;
  }
  return false;
};
